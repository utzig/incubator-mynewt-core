...
static int
hal_uart_tx_fill_fifo(struct hal_uart *u)
{
    int data = 0;
...
hal_uart_init(int port, void *arg)
{
    ...
    mask = (1 << pin_to_offset(cfg->suc_pin_tx)) | (1 << pin_to_offset(cfg->suc_pin_rx));
    GPIO_REG(GPIO_IOF_EN) |= mask;
    GPIO_REG(GPIO_IOF_SEL) &= ~mask;

    /* Install interrupt handler */
    plic_set_handler(INT_UART0_BASE, uart_irq_handler, 3);
